...
const required = [
  ...
  ['target-local normalization helper exists', coordinates, 'targetLocalToClientPoint'],
  ['target-local pointer offsets are canonical Safari refinement input', canonical, 'event.offsetX'],
  ['target-local refinement pointer is forwarded before frozen editor handling', canonical, 'forwardTargetLocalRefinementPointer'],
  ['refinement surface has an explicit adapter boundary', canonical, 'REFINEMENT_SURFACE_SELECTOR'],
  ['normalized refinement event uses target-local client coordinates', canonical, 'clientX: client.x'],
  ['real refinement event is stopped before duplicate editor handling', canonical, 'event.stopImmediatePropagation()'],
  ['synthetic normalized refinement events do not recurse', canonical, 'if (!event.isTrusted) return;'],
  ['iOS visual viewport remains fallback coordinate support', coordinates, 'viewportOffset: ArrangeViewportOffset = currentClientViewportOffset()'],
